...
class PartnerLoan(models.Model):
    ...
    disabled = fields.Boolean(
        string="Disabled", default=True, compute="_compute_disabled"
    )
    ...
    def _compute_name(self):
        for record in self:
            record.name = "Loan for %s on date %s" % (
    ...
    def _constraints_loan(self):
        for record in self:
            ...
            if record.amount_total < 1:
                raise ValidationError("Amount must be >= 0.1")
...
class PartnerLoan(models.Model):
    ...
    currency_id = fields.Many2one(
        "res.currency", string="Currency", related="loan_id.currency_id"
    )
